 ...
 * 
 * <p>
 * Upon instantiation, this client will establish a socket connection to a StatsD instance running on the specified host
 * and port. Metrics are then sent over this connection as they are received by the client.
 * </p>
 * 
 * <p>
 ...
 * </ul>
 * From the perspective of the application, these methods are non-blocking, with the resulting IO operations being
 * carried out in a separate thread. Furthermore, these methods are guaranteed not to throw an exception which may
 * disrupt application execution.
 * </p>
 * 
 * <p>
...
public final class NonBlockingStatsDClient implements StatsDClient {
	...
	public void count(String aspect, int delta, int samplePercentage) {
		send(String.format("%s.%s:%d|c|@%.2f", prefix, aspect, delta, samplePercentage / 100f));
	}
	...
	public void increment(String aspect, int samplePercentage) {
		count(aspect, 1, samplePercentage);
	}
}
